...
def parse_args():
    ...
    parser.add_argument("--gamma_rate_proxy", type=float, default=0.0)
    parser.add_argument("--tf_ratio_start", type=float, default=0.7)
    parser.add_argument("--tf_ratio_end", type=float, default=0.2)
    parser.add_argument("--tf_decay_epochs", type=int, default=50)
    ...
    parser.add_argument(
        ...
        type=str,
        default="pareto",
        choices=["pareto", "throughput_mae"],
        help="Criterion used to save the best checkpoint.",
    )
    parser.add_argument(
        "--no_uncertainty_loss",
        action="store_true",
        help="Use fixed multi-task loss instead of learned uncertainty weighting.",
    )
...
def differentiable_rate_proxy(rb_value, sinr_norm, sinr_mean, sinr_std):
    sinr_db = sinr_norm * sinr_std + sinr_mean
    snr_linear = torch.pow(10.0, sinr_db / 10.0)
    spectral_efficiency = torch.log2(1.0 + snr_linear).clamp(max=8.0)
    return rb_value * spectral_efficiency
...
@torch.no_grad()
def evaluate_v2_throughput_mae(model, loader, sinr_mean, sinr_std, device):
    model.eval()
    total_abs_err = 0.0
    total_sq_err = 0.0
    total_points = 0
    ...
    for rb_seq, sinr_seq, rb_label, sinr_label in loader:
        ...
        throughput_true = calculate_throughput_from_predictions_parallel(
            rb_label.reshape(-1),
            sinr_label_denorm.reshape(-1),
            device,
        )
        err = throughput_pred - throughput_true
        total_abs_err += torch.abs(err).sum().item()
        total_sq_err += (err ** 2).sum().item()
        total_points += err.numel()

    return {
        "throughput_mae": total_abs_err / total_points,
        "throughput_mse": total_sq_err / total_points,
    }
...
def main():
    ...
    if not args.no_uncertainty_loss:
        params_to_optimize += [log_var_rb, log_var_sinr]
    optimizer = torch.optim.Adam(params_to_optimize, lr=args.lr)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs)

    best_rb_acc = 0.0
    best_sinr_mae = float("inf")
    best_throughput_mae = float("inf")
    best_epoch = 0
    stale_epochs = 0
    classes = torch.arange(1, 107, device=device).float()
    tf_ratio_start = args.tf_ratio_start
    tf_ratio_end = args.tf_ratio_end
    tf_decay_epochs = max(1, args.tf_decay_epochs)
    history = []
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for rb_seq, sinr_seq, rb_label, sinr_label in pbar:
            ...
            if args.gamma_rate_proxy > 0.0:
                probs = F.softmax(rb_pred, dim=2)
                rb_expect = (probs * classes).sum(dim=2)
                rb_true = rb_label.squeeze(-1).float()
                pred_proxy = differentiable_rate_proxy(rb_expect, sinr_pred.squeeze(-1), sinr_mean, sinr_std)
                true_proxy = differentiable_rate_proxy(rb_true, sinr_label.squeeze(-1), sinr_mean, sinr_std)
                loss_rate_proxy = F.smooth_l1_loss(pred_proxy / 50.0, true_proxy / 50.0, beta=0.05)
                loss = loss + args.gamma_rate_proxy * loss_rate_proxy
        ...
        if args.selection_metric == "throughput_mae":
            eval_metrics.update(evaluate_v2_throughput_mae(model, eval_loader, sinr_mean, sinr_std, device))
        ...
        if "throughput_mae" in eval_metrics:
            msg += f" | Throughput MAE={eval_metrics['throughput_mae']:.4f}"
        print(msg)

        if args.selection_metric == "throughput_mae":
            improved = eval_metrics["throughput_mae"] < best_throughput_mae
        else:
            improved = pareto_better(
                eval_metrics["rb_acc"],
        ...
        if improved:
            ...
            if "throughput_mae" in eval_metrics:
                best_throughput_mae = min(best_throughput_mae, eval_metrics["throughput_mae"])
            best_epoch = epoch
            stale_epochs = 0
